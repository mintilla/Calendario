...
(function(){
  ...
  function injectUI(){ if(document.getElementById('vistaMapa')) return; const cont=document.querySelector('.container'); if(!cont) return; const html=`
    ...
    </div>`; const anchor=cont.querySelector('#modalOverlay'); if(anchor) anchor.insertAdjacentHTML('beforebegin',html); else cont.insertAdjacentHTML('beforeend',html); }
  ...
  async function cargarMapa(){ const wrap=document.getElementById('mapaSvgWrapper'); if(!wrap||wrap.dataset.loaded==='1') return; try{ const txt=await fetch('./world/world-geo.svg').then(r=>r.text()); wrap.innerHTML=txt; wrap.dataset.loaded='1';
      wrap.querySelectorAll('[id]').forEach(el=>{ if(el.tagName.toLowerCase()==='path' || el.tagName.toLowerCase()==='polygon' || el.tagName.toLowerCase()==='g'){ el.classList.add('country'); el.addEventListener('click', ()=> mostrarPopupPais(el.id.toUpperCase())); } });
    }catch(e){ wrap.innerHTML='<p style="color:#c00;text-align:center;">No se pudo cargar el mapa</p>'; } }

  function pintarMapa(app){ const wrap=document.getElementById('mapaSvgWrapper'); if(!wrap||!wrap.dataset.loaded) return; const d2026=dias2026PorISO(app); const hist=diasHistoricoPorISO(app); wrap.querySelectorAll('.country').forEach(el=>{ const iso=el.id.toUpperCase(); const total=(d2026[iso]?.size||0)+(hist[iso]||0); el.classList.remove('visitado-1-3','visitado-4-9','visitado-10p'); if(total>=1 && total<=3) el.classList.add('visitado-1-3'); else if(total>=4 && total<=9) el.classList.add('visitado-4-9'); else if(total>=10) el.classList.add('visitado-10p'); }); }

  function mostrarPopupPais(iso){ const app=window.calendario; if(!app) return; const nombre=Object.entries(ISO_BY_PAIS).find(([k,v])=>v===iso)?.[0]||iso; const d2026=dias2026PorISO(app); const hist=diasHistoricoPorISO(app); const total=(d2026[iso]?.size||0)+(hist[iso]||0); const viajes=(app.viajes||[]).filter(v=>ISO_BY_PAIS[v.pais]===iso); const resumen=viajes.map(v=>{ const i=new Date(v.fechaInicio), f=new Date(v.fechaFin); const dias=Math.floor((f-i)/(1000*60*60*24))+1; const destino=v.ciudadDestino?`${v.ciudadDestino} (${v.pais})`:v.pais; return `• ${destino} — ${app.formatearFechaVisual(i)} - ${app.formatearFechaVisual(f)} (${dias}d)`; }).join('<br>'); document.getElementById('mapTooltipTitle').textContent=nombre; document.getElementById('mapTooltipBody').innerHTML = `<div class="info-item" style="background:#f0f7ff;border-left:4px solid #2196F3;"><div class="info-icon">📆</div><div class="info-text"><strong>Total días:</strong> ${total} (histórico ${hist[iso]||0} + 2026 ${d2026[iso]?.size||0})</div></div>` + (resumen? `<div class="info-item"><div class="info-icon">✈️</div><div class="info-text">${resumen}</div></div>`:''); document.getElementById('mapTooltip').style.display='flex'; }
  function cerrarTooltip(){ const o=document.getElementById('mapTooltip'); if(o) o.style.display='none'; }

  function pintarHistoricoLista(app){ const cont=document.getElementById('historicoLista'); if(!cont) return; if(!app.historico||!app.historico.length){ cont.innerHTML='<p style="color:#666;">No hay registros</p>'; return;} cont.innerHTML=''; app.historico.sort((a,b)=>a.pais.localeCompare(b.pais)); app.historico.forEach((h,idx)=>{ const d=document.createElement('div'); d.className='viaje-card'; d.style.marginBottom='8px'; d.innerHTML=`<div class="viaje-card-header"><div class="viaje-destino-card">${h.pais}</div><div class="viaje-acciones"><button class="btn-editar" data-idx="${idx}">✏️ Editar</button><button class="btn-editar" data-del="${idx}" style="color:#e91e63">🗑️ Eliminar</button></div></div><div class="viaje-fechas">📆 Días previos: ${h.dias||0}</div>`; d.querySelector('[data-idx]')?.addEventListener('click', (e)=>{ const i=parseInt(e.currentTarget.getAttribute('data-idx')); const h=app.historico[i]; document.getElementById('historicoPais').value=h.pais; document.getElementById('historicoDias').value=h.dias||0; }); d.querySelector('[data-del]')?.addEventListener('click', (e)=>{ const i=parseInt(e.currentTarget.getAttribute('data-del')); if(confirm('¿Eliminar este registro histórico?')){ app.historico.splice(i,1); try{ localStorage.setItem(app.STORAGE_HISTORICO, JSON.stringify(app.historico)); }catch{} pintarHistoricoLista(app); pintarMapa(app); } }); cont.appendChild(d); }); }

  function wire(){ let tries=0,max=50; (function loop(){ tries++; const app=window.calendario; if(!app){ if(tries<max) return setTimeout(loop,100); else return; } app.STORAGE_HISTORICO=app.STORAGE_HISTORICO||'historico-viajes'; app.historico=app.historico||[]; try{ const s=localStorage.getItem(app.STORAGE_HISTORICO); if(s) app.historico=JSON.parse(s);}catch{}
    ...
    document.getElementById('btnMapaMundial')?.addEventListener('click', ()=>{ document.getElementById('vistaCalendario').style.display='none'; document.getElementById('vistaViajes').style.display='none'; document.getElementById('formularioViaje').style.display='none'; document.getElementById('vistaMapa').style.display='block'; cargarMapa().then(()=> pintarMapa(app)); });
    ...
    document.getElementById('historicoGuardar')?.addEventListener('click', ()=>{ const pais=document.getElementById('historicoPais').value; const dias=parseInt(document.getElementById('historicoDias').value||'0'); if(!pais) return alert('Selecciona un país'); if(isNaN(dias)||dias<0) return alert('Introduce días válidos'); const idx=(app.historico||[]).findIndex(x=>x.pais===pais); if(idx===-1) app.historico.push({pais,dias}); else app.historico[idx].dias=dias; try{ localStorage.setItem(app.STORAGE_HISTORICO, JSON.stringify(app.historico)); }catch{} pintarHistoricoLista(app); pintarMapa(app); });
  })(); }

  ready(function(){ ensureStyles(); injectUI(); wire(); });
})();
